...
print('\n§5.3  MATCHED achieved fill (this is the primary cost comparison)')
print(matched.pivot(index='matched_fill', columns='Model', values='cost_index')
      .rename(columns=S.NAME).to_string())
print('\n  buffer multiplier used to reach each target')
print(matched.pivot(index='matched_fill', columns='Model', values='z_used')
      .rename(columns=S.NAME).round(2).to_string())
