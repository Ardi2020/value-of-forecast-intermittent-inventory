...
# full 2024 evaluation window; all-zero units are already absent from sim_results_audit.csv
base = res[(res.markup == -1) & (res.SL == 0.95) & (res.ratio == 10) & res.lots
           & (res.term_k == 0) & (res.eval_end == '2024-12-01')]
...
ax.grid(False)
fig.tight_layout(); fig.savefig(os.path.join(FIG, 'fig4_segment_heatmap.png'), dpi=600); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(FIG, 'fig5_alignment.png'), dpi=600); plt.close(fig)
