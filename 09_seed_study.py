...
src = open(os.path.join(HERE,'ml_v2.py')).read()
out = []
for seed in (42, 7, 2026):
    ...
    subprocess.run(['python3', tmp], cwd=HERE, capture_output=True)
    subprocess.run(['python3', os.path.join(HERE,'audit_rerun.py')], cwd=HERE, capture_output=True)
